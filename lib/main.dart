...
void main() async {
  // ✅ FIXED: Correct type is WidgetsBinding (not WidgetsFlutterBinding)
  ...
  if (!kIsWeb) {
    ...
    // Mobile-only: Native splash preservation
    ...
    // Mobile-only: Hive initialization with filesystem path
    final appDocumentDirectory = await path_provider.getApplicationDocumentsDirectory();
    // ✅ FIXED: Hive.init() requires String path argument (even on web in some versions)
    Hive.init(appDocumentDirectory.path);
  } else {
    // 🌐 WEB-ONLY: Hive uses IndexedDB - pass empty string as dummy path (required by Hive API)
    Hive.init(''); // Empty string triggers web storage backend

    // Web-safe orientation handling
    SystemChrome.setPreferredOrientations([
      DeviceOrientation.portraitUp,
      DeviceOrientation.portraitDown,
    ]);
  ...
  // } else {
  //   // Web-safe error handling fallback
  ...
  // 🔒 PRODUCTION-GRADE APP CHECK
  await FirebaseAppCheck.instance.activate(
    // Android (Play Integrity - mandatory for production)
    androidProvider: AndroidProvider.playIntegrity,

    // iOS (App Attest - mandatory for production)
